...
function compress(obj) {
  ...
  const getCompressed = (part) => {
    if (Array.isArray(part)) {
      ...
      if (compressed2.endsWith("|")) compressed2 = compressed2.slice(0, -1);
      return compressed2 + "]";
    }
...
function getRelations(relations, collection, field) {
  const relation = relations.filter((relation2) => {
    return relation2.collection === collection && relation2.field === field || relation2.related_collection === collection && relation2.meta?.one_field === field;
  });
  return relation;
}
function getRelation(relations, collection, field) {
  const relation = relations.find((relation2) => {
    return relation2.collection === collection && relation2.field === field || relation2.related_collection === collection && relation2.meta?.one_field === field;
  });
  return relation;
}
...
var dynamicVariablePrefixes = ["$NOW", "$CURRENT_USER", "$CURRENT_ROLE", "$CURRENT_ROLES", "$CURRENT_POLICIES"];
  ...
  if (logicalFilterOperators.includes(key)) {
    ...
    if (logicalFilterOperators.includes(childKey)) {
      return shiftLogicalOperatorsUp({
        [childKey]: childResult[childKey].map((nestedFilter) => ({
          [key]: nestedFilter
        }))
      });
    }
...
function parseDynamicVariable(value, accountability, context) {
  ...
  if (value.startsWith("$CURRENT_POLICIES")) {
    if (value === "$CURRENT_POLICIES")
      return get2(context, value, null)?.map(({ id }) => id) ?? null;
    return get2(context, value, null);
  }
}
